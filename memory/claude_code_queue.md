...
**Décisions Robin :** (1) le room-picker **EST le hero** de l'état A ; (2) revenant avec projet sauvegardé → **bande « Reprendre ma sélection pour [pièce] »** (mini-JS d'affichage, lit localStorage, zéro filtrage) ; (3) on **garde** le champ texte libre.
...
- Bande « Reprendre » : mini-JS lit `sapiProject` ; si projet + pièce valide en état A → injecte la bande → lien `?piece=<slug>`. Affichage pur.

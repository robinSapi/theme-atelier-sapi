...
## [TÂCHE] /mes-creations/ — Refonte UX : fusion card projet + grille sélection + section "Toutes mes créations"
**Date :** 2026-05-26
**Branche :** `test-theme-sapi-maison`
**Priorité :** haute — concept validé Robin 26/05/2026, débloque un irritant UX (visiteur avec projet ne voit jamais le catalogue complet)
**Mockup de référence :** `site-web/mockups/mockup-15-mes-creations-final.html` — **source de vérité visuelle**, à OUVRIR DANS LE NAVIGATEUR et lire le code AVANT toute modification.

### Contexte

Aujourd'hui, `/mes-creations/` avec un projet actif (`sapiProject` en localStorage) filtre la grille produit silencieusement et affiche uniquement les produits qui matchent. Comportement enfermant : un visiteur avec projet ne voit JAMAIS le catalogue complet sans modifier ou supprimer son projet.

**Décision UX (validée Robin 26/05/2026) — page à 2 sections empilées :**
- **Section haute "Ma sélection"** : card englobante (pattern Conseiller, fond crème + dashed) qui contient TOUT le projet — badge "Mon projet · N luminaires" + phrase IA + signature Robin + lien Modifier + **la grille des produits matchés DANS la card** + 1 card sur-mesure en dernière cellule.
- **Section basse "Toutes mes créations"** : catalogue complet avec pills catégorie (suspensions / lampadaires / lampes à poser / appliques), pour laisser le visiteur fouiner librement sans perdre son projet.

**Sans projet** : pas de section sélection, juste une card "Conseil de Robin" qui invite à ouvrir la modale Conseiller V3, puis section "Toutes mes créations".

### Garde-fous absolus

1. **LIRE AVANT D'ÉCRIRE** : avant toute modification, lire le template actuel de `/mes-creations/` (`page-mes-creations.php` ou équivalent), le CSS conseiller, le JS conseiller. Ne PAS réinventer des classes qui existent déjà.
2. **Classes mockup ≠ classes prod** : le mockup #15 utilise des classes mockup-only (`.projet-card`, `.projet-grid`, `.projet-header`). NE PAS les reprendre telles quelles. Utiliser les classes existantes du thème (`.conseiller-bento__card`, `.conseiller-bento__badge`, `.conseiller-bento__text`, `.conseiller-bento__sig`, `.conseiller-bento__link`, etc.). Le mockup sert de référence visuelle, pas de source de code à copier.
3. **Pattern Conseiller intact** : fond crème (`--color-warm`), dashed décoratif interne (1.5px `rgba(139, 115, 85, 0.35)` inset 12px), badge wood-dark uppercase, signature Square Peg `--color-wood`, lien Modifier discret. Cf. `feedback_claude_code_refonte_grille` côté Cowork — une précédente refonte a été cassée en réécrivant hors charte.
4. **Modale Conseiller V3** : zéro impact. Le lien "Préciser ou modifier mon projet ✎" ouvre la modale V3 en mode édition (préserver le comportement actuel).
5. **Endpoints IA inchangés** : `sapi_megafilter_advice`, `sapi_megafilter_freetext`, `sapi_megafilter_chat`, etc. La phrase IA est récupérée comme aujourd'hui.
6. **H1 unique** : le hero "Pour ton [pièce]" reste l'unique H1 SEO de la page. Le badge "Mon projet" n'est pas un H2. Le titre "Toutes mes créations" peut être H2.

### Périmètre — 6 chantiers

---

### Chantier 1 — Structure HTML /mes-creations/

**Fichier :** template `/mes-creations/` (à identifier — probablement `page-mes-creations.php` ou un cas particulier dans `archive-product.php`).

**Comportement AVEC projet actif** (`sapiProject` présent côté JS, hydraté en PHP via cookie ou localStorage->server roundtrip — logique existante à conserver) :

```
[HERO existant, conservé]
  H1 Square Peg : "Pour ton [pièce]"   ← variable selon sapiProject.piece
  Sous-titre : "Ma sélection pour cette pièce"

[CARD ENGLOBANTE "Mon projet" — fond crème + dashed]
  Badge wood-dark : "Mon projet · N luminaire(s)"   ← N dynamique, singulier/pluriel auto
  Phrase IA italique (--color-wood-dark) + signature Square Peg "— Robin"
  Lien souligné : "Préciser ou modifier mon projet ✎" → ouvre modale Conseiller V3 en édition
  GRILLE de produits matchés (placée À L'INTÉRIEUR de la card crème, classes existantes thème)
    - N cards produits, ordre menu_order WooCommerce
    - + 1 card Sur-mesure en DERNIÈRE position (voir Chantier 2)

[SÉPARATEUR visuel — 3 dots en pointillés, classe à créer .sapi-section-divider ou équiv]

[SECTION "Toutes mes créations"]
  H2 "Toutes mes créations"
  Sous-titre "Le catalogue complet, classé par type de luminaire"
  Pills catégorie (Chantier 4)
  Grille catalogue complet (Chantier 3)
```

**Comportement SANS projet** (`sapiProject` absent) :

```
[HERO]
  H1 Square Peg : "Mes créations"
  Sous-titre : "Tous les luminaires de l'atelier"

[CARD "Conseil de Robin" — pattern Conseiller, badge wood-dark]
  Badge : "Conseil de Robin"
  Phrase invitation : "Avant de tout regarder, dis-moi pour quelle pièce tu cherches — je te ferai une sélection adaptée à ton projet." + signature
  CTA orange (`--color-orange`) : "Démarrer mon projet" → ouvre modale Conseiller V3 en création

[SECTION "Toutes mes créations"]
  ...identique au cas avec projet
```

**Cas dégradé : projet actif MAIS 0 match** (ex: salle de réunion + style baroque) :
Card englobante affichée normalement (badge, phrase IA, lien Modifier) MAIS sans grille produits. À la place, **uniquement la card sur-mesure** (en pleine cellule centrée). Comportement à raffiner plus tard, on accepte le cas dégradé en V1.

---

### Chantier 2 — Card sur-mesure (format cellule de grille)

**Position :** dernière cellule de la grille de la section "Ma sélection" UNIQUEMENT. Pas dans "Toutes mes créations".

**Référence visuelle :** mockup #15 ligne ~230 (`.card--surmesure`).

**Structure :**
- Format cellule de grille (mêmes dimensions que les cards produits standards)
- Zone photo (aspect ratio 1/1.1) : fond dégradé orange clair `linear-gradient(135deg, #FFF4ED 0%, #FFE4D3 100%)`, icône SVG recyclage centrée (42×42px, stroke `--color-orange`), badge pill orange "Sur-mesure" sous l'icône
- Zone body : titre uppercase `--color-orange` "Et si on créait le tien ?" + sous-titre italique `--color-wood-mid` "Décris ton projet à Robin"
- Bordure dashed orange légère `1px dashed rgba(227, 91, 36, 0.45)` (subtile différence avec les cards produit)
- Au clic : `href="/sur-mesure/"`

**Wording (validé Robin)** : "Et si on créait le tien ?" / "Décris ton projet à Robin".

---

### Chantier 3 — Logique de filtrage et requêtes WP_Query

**Section haute "Ma sélection" :** récupérer la liste des produits qui matchent `sapiProject.answers` via la **logique de filtrage existante** (le filtre qui s'applique aujourd'hui à toute la page /mes-creations/). Factoriser cette logique dans une fonction réutilisable si pas déjà fait.

**Section basse "Toutes mes créations" :** TOUS les produits publiés, ordre `menu_order` WooCommerce, AUCUN filtre projet appliqué. C'est ce filtre absent qui fait la nouveauté.

**Important :** s'assurer que les deux WP_Query coexistent proprement :
- Pas de double pagination
- Aucun hook `pre_get_posts` ou filtre WooCommerce qui s'applique aux deux par erreur
- Comptage des produits matchés pour le badge "Mon projet · N luminaires"

---

### Chantier 4 — Pills catégorie (récupération git history pré-F1)

**Action :**
1. Identifier dans le git history le composant pills catégorie qui existait sur `/mes-creations/` AVANT la suppression effectuée en F1 (chantier "F1a — câblage home → /mes-creations/?piece=" ou voisins, fin avril / début mai 2026). Recherche : `git log --all --oneline -- "*page-mes-creations*"` puis `git show` sur les commits pertinents. Chercher aussi `product_cat` ou `chip` dans les diffs.
2. Récupérer le HTML, le CSS et le JS associés (probablement classes type `.megafilter-bar`, `.chip-product-cat` ou équivalent — à confirmer dans le history).
3. Réintégrer dans la section "Toutes mes créations" UNIQUEMENT.
4. Catégories à afficher : **Tous** (actif par défaut), **Suspensions**, **Lampadaires**, **Lampes à poser**, **Appliques** — compteurs dynamiques depuis WC (`get_terms('product_cat')` avec `count`).
5. **Comportement clic** : filtrage **AJAX sans rechargement** (préserve la card projet en haut). Mise à jour URL via `history.pushState` avec `?product_cat=<slug>`. Au reload avec param présent, appliquer le filtre automatiquement côté serveur.

Si le design git-history pré-F1 n'est pas raisonnable à remettre tel quel (CSS cassé, dépendances supprimées), faire le minimum nécessaire pour avoir des pills sobres et fonctionnelles — pattern visible dans le mockup #15 comme fallback.

---

### Chantier 5 — Tracking GA4

Events à émettre via `gtag('event', ...)` (ou via dataLayer.push si GTM4WP est privilégié) :

- `mes_creations_card_selection_click` — clic sur une card de la section "Ma sélection". Params : `product_id`, `piece`, `position` (1..N)
- `mes_creations_card_catalogue_click` — clic sur une card de "Toutes mes créations". Params : `product_id`, `product_cat`, `position`
- `mes_creations_pill_categorie_click` — clic sur une pill cat. Params : `categorie_slug`
- `mes_creations_card_surmesure_click` — clic sur la card sur-mesure de la sélection. Params : `piece`
- `mes_creations_lien_modifier_projet_click` — clic sur "Préciser ou modifier mon projet". Params : `piece`

Cohérent avec le tracking V3 défini dans la tâche "Refonte page admin Robin Conseiller V3" (ne pas dédoubler les events si certains existent déjà — à vérifier).

---

### Chantier 6 — Responsive mobile

Cohérent avec mockup #15 (`@media max-width: 768px`) :

- Card englobante "Mon projet" : padding interne réduit `34px 18px 30px`, dashed inset 8px (au lieu de 12px)
- Grille dans la card : `grid-template-columns: repeat(2, 1fr)` (forcer 2 colonnes, le auto-fill 220px ne tient pas en mobile étroit)
- Hero : padding `70px 20px 36px`
- Pills cat : `flex-wrap: nowrap` + `overflow-x: auto`, scrollbar masquée (`scrollbar-width: none` + `::-webkit-scrollbar { display: none }`), padding ajusté
- Phrase IA : `font-size: 16px` (au lieu de 17px)

---

### Critères de succès

- [ ] Rendu desktop fidèle à la STRUCTURE du mockup #15 (pas pixel-perfect — fidélité structurelle suffit, Robin itérera sur les pixels)
- [ ] Rendu mobile (test à 375px) propre : grille 2 colonnes dans la card englobante, pills scroll horizontal sans scrollbar visible
- [ ] **Avec projet "salon"** : hero "Pour ton salon" + card englobante avec grille des matches + card sur-mesure en dernière cellule + séparateur + section "Toutes mes créations" avec pills cat fonctionnelles
- [ ] **Sans projet** : hero "Mes créations" + card "Conseil de Robin" avec CTA orange "Démarrer mon projet" + section "Toutes mes créations"
- [ ] Lien "Préciser ou modifier mon projet ✎" ouvre la modale Conseiller V3 en mode édition
- [ ] CTA "Démarrer mon projet" ouvre la modale Conseiller V3 en mode création
- [ ] Pills cat : filtrage AJAX, pas de reload, URL mise à jour avec `?product_cat=<slug>`
- [ ] Au reload avec `?product_cat=suspensions`, la pill "Suspensions" est active et la grille filtrée
- [ ] Compteurs pills dynamiques (vrais comptes WC, pas hardcodés)
- [ ] Card sur-mesure visible UNIQUEMENT dans "Ma sélection" (pas dans "Toutes mes créations")
- [ ] Badge "Mon projet · N luminaire(s)" affiche le bon N avec singulier/pluriel
- [ ] Events GA4 vérifiables en console + GA4 DebugView
- [ ] Aucune régression sur la modale Conseiller V3
- [ ] Aucune régression sur les endpoints IA `sapi_megafilter_*`
- [ ] Pages `/categorie-produit/*` INCHANGÉES (hors scope de cette tâche)

### Notes

- **Pas de phrase IA dédiée pour le cas "0 match"** : on accepte la card englobante "vidée" avec uniquement la card sur-mesure visible. À raffiner dans un ticket séparé si le cas devient fréquent.
- **Pages catégorie** (`/categorie-produit/suspensions/`, etc.) : strictement hors scope. La logique double-grille est appliquée UNIQUEMENT sur `/mes-creations/`.
- **Branche** : code sur `test-theme-sapi-maison`, auto-déploiement sur `test.atelier-sapi.fr` via GitHub Actions, validation Robin sur test AVANT merge vers master.
